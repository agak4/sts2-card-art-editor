...
func normalize_image_with_adjustment(image, target_size: Vector2i, zoom: float, offset_x: float, offset_y: float):
	...
	var scale_factor = max(scale_x, scale_y) * zoom

	var resized_width = int(round(working_image.get_width() * scale_factor))
	var resized_height = int(round(working_image.get_height() * scale_factor))
	...
	var dest_x = max(0, (target_size.x - resized_width) / 2 + int(round(offset_x * max(0, target_size.x - resized_width) * 0.5)))
	var dest_y = max(0, (target_size.y - resized_height) / 2 + int(round(offset_y * max(0, target_size.y - resized_height) * 0.5)))
	
	if resized_width >= target_size.x and resized_height >= target_size.y:
		...
		normalized_image.blit_rect(
			working_image,
			Rect2i(0, 0, working_image.get_width(), working_image.get_height()),
